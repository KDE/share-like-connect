...
KIPIInterface::KIPIInterface(KipiProvider* provider)
:KIPI::Interface(provider)
...
KIPI::ImageCollection KIPIInterface::currentAlbum() {
    return KIPI::ImageCollection(new ProviderImageCollectionShared(d->provider));
}
...
KIPI::ImageInfo KIPIInterface::info(const KUrl& url) {
    return KIPI::ImageInfo(new ProviderInfoShared(this,url));
}
